...
void FormController::service(HttpRequest& request, HttpResponse& response)
{
    ...
    if (request.getParameter("action")=="show")
    {
        ...
        auto input_string = QString(QString(request.getParameter("input")).toUtf8()).toStdString();
        ...
        std::cout<<5;
        std::cout<<input_string;
        std::string path_to___="D:\\QTWebApp-Lemmatizer\\Demo1";
std::cout<<2;
...
std::cout<<4;
        ...
        std::cout<<7;
            ...
            analyzer::grab_dict(path_to___ + "\\1" + "sport" + ".txt");
           // std::cout<<input_string.toStdString();
            _math_core.analyze_one_text(input_string);
            //_math_core.find_SVD_terms("sport", input_string);
        ...
        std::string res = analyzer::get_text();
        for(auto s:res)
        {
            data_for_write.append(QString(s));
