...
void math_core::analyze_one_text(string _path)
{
	...
	this->max_cont_size = analyzer::counter_of_tokenizer_without_rare_words;
}
...
void math_core::calculate_all()
{
	...
	for (int i = 0; i < this->number_of_slices; ++i) {
		...
		std::cout.flush();
		cout << endl << "Вычисление математического ожидания...";	//1
		this->calculate_mat_ozidanie();
		std::cout.flush();
		cout << endl << "Вычисление математической дисперсии...";	//2
		this->calculate_mat_disperse();
		std::cout.flush();
		cout << endl << "Вычисление флуктуационных коллокаций...";	//3
		this->find_fluctuations();
		std::cout.flush();
		cout << endl << "Сокращение числа флуктуационных коллокаций...";	//4
		this->shrink_set_of_fluct_cooloc();
		std::cout.flush();
		cout << endl << "Сокращение контейнера математического ожидания...";	//5
		this->shrink_mat_ozid();
		std::cout.flush();
		cout << endl << "Вычисление нормированного математического ожидания...";	//7
		this->calculate_norm_shrinked_mat_ozid();
		std::cout.flush();
		cout << endl << "Вычисление tf и idf...";	//8
		this->calculate_tf_idf();
		cout << endl << "SVD разложение tf матрицы...";	//9
...
void math_core::SVD_colloc_algorithm(now_type* arr, size_t rows, bool is_this_SVD_for_terms)
{
	...
	if (is_this_SVD_for_terms) {
		...
		for (auto i = global_var::KOEF_FOR_COLLOC_COS_DELETE; i < (1.f - 0.01f); i += 0.01f) {
			...
			std::cout.flush();
			cout << endl << "Количество значимых термов/коллокаций после разложения и свертки tf/idf, с множителем " << i << " :" << this->colloc_and_terms_after_SVD->size();
...
void math_core::find_SVD_terms(string path_to_db, string _path___)
{
	...
	ifstream _input(global_var::SUPER_PATH + "\\" + path_to_db + ".txt");
	...
	for (auto obj : terms) {
		...
		(*this->tf_matrix)[stoi(vec[0])][stoi(vec[1])] = stof(vec[2]);
	}

	parser _parser(_path___);	//tut peredaetsa kopiya
	auto result_of_parse = _parser.parse();

	analyzer _analyzer(result_of_parse);
	_analyzer.calculate_tf_matrix_for_only_terms(50);
	...
	for (size_t i = 0; i < this->idf_matrix->size(); ++i)
		(*this->tf_matrix)[i][50] *= (*this->idf_matrix)[i];
	...
	this->SVD_colloc_algorithm(tf_like_row.data(), tf_like_row.size() / 51, true);

	tf_like_row.clear();

	analyzer::vost_map_of_tokens_Word_and_number_of_appearances_struct_TOKEN_();

	//cout << endl << "Количество значимых термов после разложения tf/idf, с множителем " << global_var::KOEF_FOR_COLLOC_COS_DELETE << " :" << this->cosinuses.size();
	...
	auto _set = analyzer::parse_and_lematize_one_text(_path___);

	list<int> _for_del;

	for (auto obj : *this->set_for_unique_terms)
		if (_set->find(obj) == _set->end())
			_for_del.push_back(obj);

	for (auto obj : _for_del)
		this->set_for_unique_terms->erase(obj);


	this->cosinuses.clear();
	//std::cout.flush();
	//cout << endl << "Количество значимых термов после чистки " << global_var::KOEF_FOR_COLLOC_COS_DELETE << " :" << this->set_for_unique_terms->size();

	//cout << endl << endl;
	analyzer::out_of_predl(this->set_for_unique_terms, _path___);
...
void math_core::prepare_tf_matrix_for_SVD()
{
	...
	cout << endl << "Количество значимых термов/коллокаций после разложения tf/idf, с множителем " << 0.99 << " :" << this->cosinuses.size();
	...
	this->cosinuses.clear();
	std::cout.flush();
	cout << endl << "Количество значимых термов/коллокаций после разложения и свертки tf/idf, с множителем " << 0.99 << " :" << this->colloc_and_terms_after_SVD->size();
	...
	for (auto obj : *this->colloc_and_terms_after_SVD)
		if (obj.second_coord != -1) {
			...
			/*if (parser::stop_words.find(analyzer::get_word_for_token(obj.first_coord)) != parser::stop_words.end() || (parser::stop_words.find(analyzer::get_word_for_token(obj.second_coord)) != parser::stop_words.end()))
				cout << endl << "Произошло страшное. В вывод проникло стоп-слово";

			if ((analyzer::get_word_for_token(obj.first_coord).find("И") != string::npos && (analyzer::get_word_for_token(obj.first_coord).size() >= 2 && analyzer::get_word_for_token(obj.first_coord).size() <= 4))) {
				cout << endl << "Подозрительные символы: ";
				for (auto obj : analyzer::get_word_for_token(obj.first_coord))
					cout << (int)obj << " ";
			}

			if((analyzer::get_word_for_token(obj.second_coord).find("И") != string::npos && (analyzer::get_word_for_token(obj.second_coord).size() >= 2 && analyzer::get_word_for_token(obj.second_coord).size() <= 4))) {
				cout << endl << "Подозрительные символы: ";
